...
export const RepositoryProgressPage = () => {
  ...
  return <div className="max-w-7xl space-y-6">
    ...
    {!isLoading && snapshots.length === 0 && !error ? <Card><CardContent className="p-10 text-center text-sm text-slate-500">Repository này chưa có snapshot. Hãy phân tích lại repository để tạo mốc theo dõi đầu tiên.</CardContent></Card> : <>
      ...
      {latest?.skillVectorSummary && <Card><CardHeader><CardTitle>Tóm tắt skill vector</CardTitle><CardDescription>Phân nhóm kỹ năng được ghi nhận trong snapshot gần nhất.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><div className="rounded-lg border border-slate-200 p-3 dark:border-slate-800"><p className="text-xs text-slate-500">Tổng kỹ năng</p><p className="mt-1 text-xl font-semibold">{latest.skillVectorSummary.totalSkills}</p></div><div className="rounded-lg border border-emerald-200 p-3 dark:border-emerald-900"><p className="text-xs text-slate-500">Mạnh</p><p className="mt-1 text-xl font-semibold text-emerald-600">{latest.skillVectorSummary.strongCount}</p></div><div className="rounded-lg border border-cyan-200 p-3 dark:border-cyan-900"><p className="text-xs text-slate-500">Đang phát triển</p><p className="mt-1 text-xl font-semibold text-cyan-600">{latest.skillVectorSummary.developingCount}</p></div><div className="rounded-lg border border-amber-200 p-3 dark:border-amber-900"><p className="text-xs text-slate-500">Cần củng cố</p><p className="mt-1 text-xl font-semibold text-amber-600">{latest.skillVectorSummary.weakCount}</p></div><div className="rounded-lg border border-red-200 p-3 dark:border-red-900"><p className="text-xs text-slate-500">Thiếu</p><p className="mt-1 text-xl font-semibold text-red-600">{latest.skillVectorSummary.missingCount}</p></div></CardContent></Card>}
